    ...
    <div class="max-w-[1440px] mt-[20px] p-[40px] border-orange-600 max-h-full w-full h-full mx-auto text-white">
        <div class="">
            <div>
                ...
                <span class="text-neutral-500">You can add Category & Brand here.</span>
            </div>
        </div>
        <div class="flex w-full space-x-8 h-fit">
            ...
            <div class="w-full h-full">
                <div class="mt-[35px] flex items-center space-x-8">
                    <h1 class="text-[25px] font-semibold underline underline-offset-4">Add Category</h1>
                    <%
                        if (session.getAttribute("categorySuccessMessage") != null) {
                    %>
                        <div class="my-auto text-green-500 font-semibold">
                            <%=(String) session.getAttribute("categorySuccessMessage")%>
                        </div>
                    <%
                            session.removeAttribute("categorySuccessMessage");
                        }
                    %>
                    <%
                        if (session.getAttribute("categoryErrorMessage") != null) {
                    %>
                    <div class="my-auto text-red-500 font-semibold">
                        <%=(String) session.getAttribute("categoryErrorMessage")%>
                    </div>
                    <%
                            session.removeAttribute("categoryErrorMessage");
                        }
                    %>
                </div>
                <form action="<%=request.getContextPath()%>/create-product-category" method="post">
                    <div class="flex max-w-[1440px] w-full">
                        <div class="w-full">
                            <div class="space-y-[8px] font-semibold text-xl mt-3">
                                ...
                                <br />
                                <input type="text" name="category_name" id="category_name" class="form-input !max-w-full" placeholder="Name" required />
                            </div>
                            <div class="space-y-[8px] text-white font-semibold text-xl mt-3">
                                ...
                                <br />
                                <textarea name="category_description" id="description" class="form-input !max-w-full" rows="15" placeholder="Description" required></textarea>
                            </div>
                        </div>
                    </div>
            ...
            <div class="w-full h-full">
                <div class="max-w-[1300px] border-orange-600 max-h-full w-full mt-[20px] mb-[40px]">
                    <div class="mt-[35px] flex items-center space-x-8">
                        <h1 class="text-[25px] font-semibold underline underline-offset-4">Add Brand</h1>
                        <%
                            if (session.getAttribute("brandSuccessMessage") != null) {
                        %>
                        <div class="my-auto text-green-500 font-semibold">
                            <%=(String) session.getAttribute("brandSuccessMessage")%>
                        </div>
                        <%
                                session.removeAttribute("brandSuccessMessage");
                            }
                        %>
                        <%
                            if (session.getAttribute("brandErrorMessage") != null) {
                        %>
                        <div class="my-auto text-red-500 font-semibold">
                            <%=(String) session.getAttribute("brandErrorMessage")%>
                        </div>
                        <%
                                session.removeAttribute("brandErrorMessage");
                            }
                        %>
                    </div>
                    <form action="<%= request.getContextPath()%>/add-brand" method="post" enctype="multipart/form-data">
                        <div class="max-w-[1270px] w-full space-y-4">
                            <div class=" !max-w-[1000px] w-full mx-auto">
                                <div class="space-y-[8px] text-white font-semibold text-xl mt-3">
                                    ...
                                    <br />
                                    <input type="text" name="brand_name" class="form-input !max-w-full" placeholder="Name" id="brand_name" required />
                                </div>
                            </div>
